...
def build_trade_reconstruction(strategy_name: str, df: pd.DataFrame, portfolio_value_series: pd.Series) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    alloc_df = build_allocation_frame(strategy_name, df)
    if alloc_df.empty:
        empty = pd.DataFrame()
    ...
    for i, dt in enumerate(rebalance_dates):
        ...
        # current marked-to-market holdings before applying the new target
    ...
    # final mark-to-market on end date using last reconstructed shares
    ...
    summary_rows.append({
        "Date": pd.to_datetime(end_dt).strftime("%Y-%m-%d"),
        "State": state_series.loc[end_dt],
        "Portfolio Value Before": float(portfolio_value_series.loc[end_dt]),
        "Portfolio Value After": float(portfolio_value_series.loc[end_dt]),
        "Cash Before": cash_balance,
...
if run:
    ...
    trade_detail_df, trade_summary_df, final_positions_df = build_trade_reconstruction(strategy, df, eq_series)
    ...
    if not trade_summary_df.empty:
        ...
        x1.markdown(metric_card_html("Holdings-Based Final Value", f"${reconstructed_final:,.0f}", ["From reconstructed share counts", "Uses official close prices"]), unsafe_allow_html=True)
        ...
        x3.markdown(metric_card_html("Reconstruction Difference", f"${diff:,.2f}", ["Reconstructed minus backtest", "Should shrink materially after timing fix"]), unsafe_allow_html=True)
