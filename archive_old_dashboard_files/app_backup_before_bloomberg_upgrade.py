...
def build_regime_fig(strategy_name: str, df: pd.DataFrame) -> go.Figure:
    ...
    regime_colors = {"R1 Risk-On": "#34d399", "R2 Bear": "#f87171", "R3 Growth": "#60a5fa", "R4 Defensive": "#fbbf24"}
...
st.set_page_config(page_title="ADAPT Strategy Terminal", layout="wide", initial_sidebar_state="expanded")
...
st.markdown(
    ...
    h1 { font-size: 2.45rem !important; font-weight: 800 !important; letter-spacing: -0.03em; margin-bottom: 0.2rem !important; }
    h2, h3 { font-weight: 740 !important; letter-spacing: -0.02em; }
    ...
    .eyebrow { text-transform: uppercase; letter-spacing: 0.14em; font-size: 0.78rem; color: var(--gold); font-weight: 700; margin-bottom: 0.45rem; }
    .hero-sub { color: var(--muted); font-size: 1rem; margin-top: 0.25rem; }
    .hero-pill-wrap { display: flex; flex-wrap: wrap; gap: 10px; justify-content: flex-end; }
    .hero-pill {
    ...
    .metric-label { text-transform: uppercase; letter-spacing: 0.12em; font-size: 0.74rem; color: #93a4bd; font-weight: 700; margin-bottom: 8px; }
    .metric-value { font-size: 2.0rem; line-height: 1.0; font-weight: 800; color: #f8fbff; margin-bottom: 8px; }
    .metric-sub { font-size: 0.88rem; line-height: 1.35; color: #9fb0c7; }
    ...
    .note-box { margin-top: 6px; margin-bottom: 6px; }
...
with st.sidebar:
    ...
    change_log_sort = st.selectbox("Allocation Change Log Order", ["Newest to Oldest", "Oldest to Newest"], index=0)
...
if run:
    ...
    equity_fig, total_contributions, strategy_equity_s, strategy_dd_s = build_equity_drawdown_fig(
        strategy_choice, df, bench_ret, start_capital, contribution_amount, contribution_frequency, hover_mode
    )
    ...
    st.markdown(
        ...
        {strategy_choice} | {start_date} to {end_date} | {len(df):,} trading days | Estimated allocation trades: <strong>{trade_count:,}</strong>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with mid4:
        st.markdown(
            ...
            <div class="metric-shell">
              ...
              <div class="metric-sub">Allocation changes in selected period<br>See detailed log below</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with row4_right:
        ...
        st.dataframe(
            benchmark_table(strategy_choice, df["ret"], bench_ret, start_capital, contribution_amount, contribution_frequency),
