...
def metric_grid_html(cards: list[str]) -> str:
    joined = "".join(f'<div class="metricgrid-item">{card}</div>' for card in cards)
    return f'<div class="metricgrid">{joined}</div>'
...
st.markdown(
    ...
    .metricgrid { display:grid; grid-template-columns:repeat(auto-fit, minmax(260px, 1fr)); gap:14px; margin: 0 0 14px 0; align-items:stretch; }
    .metricgrid-item { min-width:0; }
...
if run:
    ...
    overview_cards = [
        metric_card_html("CAGR", f"{perf['cagr']:.2%}", [f"{perf['days']:,} trading days", 'Calculated from df["ret"]']),
        metric_card_html("Max Drawdown", f"{perf['maxdd']:.2%}", [f"Ulcer Index {ulcer:.2%}", "Recovery metrics below"]),
        metric_card_html("Sharpe", f"{perf['sharpe']:.2f}", [f"Calmar {perf['calmar']:.2f}", "Return-stream metric"]),
        metric_card_html("Final Value", f"${eq_series.iloc[-1]:,.0f}", [f"Contributions ${total_contrib:,.0f}", "Equity includes contribution schedule"]),
        metric_card_html("Median Recovery", f"{0 if median_rec is None else median_rec:.0f} d", [f"Average {0 if avg_rec is None else avg_rec:.1f} d", f"Longest {0 if max_rec is None else max_rec:.0f} d"]),
        metric_card_html("Rolling 3Y CAGR", f"{roll3_med:.2%}", [f"Best {roll3_best:.2%}", f"Worst {roll3_worst:.2%}"]),
        metric_card_html("2Y Underperformance", f"{prob_neg:.2%}", ["Prob(2Y CAGR < 0%)", f"Median 2Y CAGR {roll2_med:.2%}"], tooltip=under_tooltip, align="left"),
        metric_card_html("Sub-Target Risk", f"{prob_sub_target:.2%}", [f"Prob(<{sub_target_threshold:.1f}%) over rolling 2Y periods", f"Rebalance events {rebalance_events:,}"], tooltip=sub_target_tooltip, align="right"),
    ]
    st.markdown(metric_grid_html(overview_cards), unsafe_allow_html=True)
