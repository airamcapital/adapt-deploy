...
st.markdown(
    ...
    .hero { background: linear-gradient(135deg, #0f172a 0%, #111f3d 100%); border: 1px solid #334155; border-radius: 18px; padding: 14px 20px 20px 20px; margin-bottom: 16px; overflow: visible; }
    .metricbox { background: #0f172a; border: 1px solid #334155; border-radius: 14px; padding: 14px; min-height: 220px; display:flex; flex-direction:column; justify-content:flex-start; }
    ...
    .tooltip-box { visibility:hidden; opacity:0; position:absolute; top:24px; right:0; width:340px; background:#0f172a; border:1px solid #334155; color:#e2e8f0; border-radius:10px; padding:10px 12px; font-size:0.85rem; line-height:1.4; z-index:9999; transition:opacity 0.15s ease; box-shadow:0 10px 24px rgba(0,0,0,0.35); }
...
if run:
    ...
    with c7:
        st.markdown(tooltip_label("2Y Underperformance", under_tooltip), unsafe_allow_html=True)
        st.markdown(f'<div class="metricbox"><div class="metricvalue">{prob_neg:.2%}</div><div class="metricsub">Prob(2Y CAGR &lt; 0%)<br>Median 2Y CAGR {roll2_med:.2%}<br>{prob_neg:.2%} of rolling 2Y periods were negative</div></div>', unsafe_allow_html=True)
    with c8:
        st.markdown(tooltip_label("Sub-Target Risk", sub_target_tooltip), unsafe_allow_html=True)
        st.markdown(f'<div class="metricbox"><div class="metricvalue">{prob_sub_target:.2%}</div><div class="metricsub">Prob(&lt;{sub_target_threshold:.1f}%) over rolling 2Y periods<br>Rebalance events {rebalance_events:,}<br>{prob_sub_target:.2%} below threshold</div></div>', unsafe_allow_html=True)
    ...
    st.markdown(tooltip_label("Current Allocation", "This table shows the current live allocation for the selected strategy and end date.<br><br><strong>Risk On</strong>: more aggressive exposure.<br><strong>Neutral</strong>: balanced intermediate stance.<br><strong>Risk Off</strong>: preservation-oriented stance.", f"Current state: {state_now}"), unsafe_allow_html=True)
