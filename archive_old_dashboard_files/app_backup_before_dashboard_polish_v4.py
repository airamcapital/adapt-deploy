...
st.markdown(
    ...
    .hero { background: linear-gradient(135deg, #0f172a 0%, #111f3d 100%); border: 1px solid #334155; border-radius: 18px; padding: 34px 20px 18px 20px; margin-bottom: 14px; overflow: visible; }
    .metricbox { background: #0f172a; border: 1px solid #334155; border-radius: 14px; padding: 10px 12px; min-height: 108px; display:flex; flex-direction:column; justify-content:flex-start; }
...
st.markdown(
    """
    <div class="hero">
      <div style="color:#fbbf24;font-size:0.78rem;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;">ADAPT Strategy Terminal</div>
      <h1 style="margin:6px 0 4px 0; font-size:3.0rem; line-height:1.06;">Institutional Strategy Terminal</h1>
      <div style="color:#94a3b8;font-size:0.96rem;">Institutional analytics for the ADAPT CORE + ALPHA system. Display layer only — preserves the validated ADAPT engine workflow and return stream calculations.</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
if run:
    ...
    st.markdown('<div style="font-size:2.0rem; font-weight:700; margin:0 0 10px 0;">' + ("COMBINED Overview" if strategy == "COMBINED" else f"{strategy} Overview") + '</div>', unsafe_allow_html=True)
    c1, c2, c3, c4 = st.columns(4)
    c1.markdown(metric_card_html("CAGR", f"{perf['cagr']:.2%}", [f"{perf['days']:,} trading days", 'Calculated from df["ret"]']), unsafe_allow_html=True)
    c2.markdown(metric_card_html("Max Drawdown", f"{perf['maxdd']:.2%}", [f"Ulcer Index {ulcer:.2%}", "Recovery metrics below"]), unsafe_allow_html=True)
    c3.markdown(metric_card_html("Sharpe", f"{perf['sharpe']:.2f}", [f"Calmar {perf['calmar']:.2f}", "Return-stream metric"]), unsafe_allow_html=True)
    c4.markdown(metric_card_html("Final Value", f"${eq_series.iloc[-1]:,.0f}", [f"Contributions ${total_contrib:,.0f}", "Equity includes contribution schedule"]), unsafe_allow_html=True)
    ...
    c5.markdown(metric_card_html("Median Recovery", f"{0 if median_rec is None else median_rec:.0f} d", [f"Average {0 if avg_rec is None else avg_rec:.1f} d", f"Longest {0 if max_rec is None else max_rec:.0f} d"]), unsafe_allow_html=True)
    c6.markdown(metric_card_html("Rolling 3Y CAGR", f"{roll3_med:.2%}", [f"Best {roll3_best:.2%}", f"Worst {roll3_worst:.2%}"]), unsafe_allow_html=True)
    c7.markdown(metric_card_html("2Y Underperformance", f"{prob_neg:.2%}", ["Prob(2Y CAGR < 0%)", f"Median 2Y CAGR {roll2_med:.2%}", f"{prob_neg:.2%} of rolling 2Y periods were negative"], tooltip=under_tooltip, align="left"), unsafe_allow_html=True)
    c8.markdown(metric_card_html("Sub-Target Risk", f"{prob_sub_target:.2%}", [f"Prob(<{sub_target_threshold:.1f}%) over rolling 2Y periods", f"Rebalance events {rebalance_events:,}", f"{prob_sub_target:.2%} below threshold"], tooltip=sub_target_tooltip, align="right"), unsafe_allow_html=True)
