...
def curve_from_returns_with_contributions(returns: pd.Series, start_value: float, contribution_amount: float = 0.0, contribution_frequency: str = "None") -> tuple[list[float], list[float], float]:
...
def build_holdings_text(strategy_name: str, df: pd.DataFrame) -> list[str]:
    ...
    if strategy_name == "CORE":
        ...
        for _, row in df.iterrows():
            ...
            texts.append(alloc_to_text(combine_allocs(core_alloc, alpha_alloc, float(row["core_w"]), float(row["alpha_w"]))))
    else:
...
def get_state_distribution(strategy_name: str, df: pd.DataFrame) -> tuple[pd.Series, str, str]:
    if strategy_name == "CORE":
        ...
        tooltip = (
            "Shows the share of days spent in each CORE regime state.<br><br>"
            "<strong>R1 Risk-On</strong>: higher equity exposure in favorable conditions.<br>"
            "<strong>R2 Bear</strong>: inverse/risk-off bear regime.<br>"
            "<strong>R3 Growth</strong>: moderate growth-oriented exposure in trending conditions.<br>"
            "<strong>R4 Defensive</strong>: preservation-oriented allocation in risk-off conditions."
        )
    ...
    if strategy_name == "ALPHA":
        ...
        tooltip = (
            "Shows the share of days spent in each ALPHA state.<br><br>"
            "<strong>ALPHA Risk-On</strong>: ALPHA sleeve allocated to active risk exposure.<br>"
            "<strong>ALPHA Cash</strong>: ALPHA sleeve parked in cash / no active long exposure."
        )
        return counts, "ALPHA State Distribution", tooltip

    if "core_regime" in df.columns:
        labels = {1: "R1 Risk-On", 2: "R2 Bear", 3: "R3 Growth", 4: "R4 Defensive"}
        counts = df["core_regime"].astype(int).map(labels).value_counts()
        tooltip = (
            "Shows the share of days spent in each COMBINED engine regime state.<br><br>"
            "This reflects the combined system's regime classification logic used to drive the combined allocation layer."
        )
        return counts, "COMBINED State Distribution", tooltip
...
def build_equity_drawdown_fig(strategy_name: str, df: pd.DataFrame, bench_ret: pd.DataFrame, start_value: float, contribution_amount: float, contribution_frequency: str, hover_mode: str = "Performance") -> tuple[go.Figure, float, pd.Series, pd.Series]:
...
def build_state_fig(counts: pd.Series) -> go.Figure:
    if counts.empty:
        return go.Figure()
    color_map = {"R1 Risk-On": "#34d399", "R2 Bear": "#f87171", "R3 Growth": "#60a5fa", "R4 Defensive": "#fbbf24", "ALPHA Risk-On": "#34d399", "ALPHA Cash": "#94a3b8"}
...
.stApp {
    background:
        radial-gradient(circle at top right, rgba(251,191,36,0.07), transparent 24%),
        radial-gradient(circle at left, rgba(96,165,250,0.06), transparent 20%),
        linear-gradient(180deg, #050d19 0%, #091223 100%);
    color: var(--text);
...
h3 { font-size: 1.85rem !important; font-weight: 800 !important; margin-bottom: 0.35rem !important; }
...
.hero-sub { color: var(--muted); font-size: 1.02rem; line-height: 1.55; max-width: 820px; }
.hero-pill-wrap { display: flex; flex-wrap: wrap; gap: 10px; justify-content: flex-end; max-width: 500px; }
...
.tooltip-label { display: inline-flex; align-items: center; gap: 8px; font-size: 1.25rem; font-weight: 760; letter-spacing: -0.02em; color: #edf3fb; margin-bottom: 8px; }
...
.tooltip-text { visibility: hidden; opacity: 0; position: absolute; right: 24px; left: auto; top: 24px; width: 320px; background: #0a1220; color: #e6eef9; border: 1px solid rgba(255,255,255,0.12); border-radius: 12px; padding: 11px 12px; box-shadow: 0 12px 28px rgba(0,0,0,0.28); font-size: 0.84rem; line-height: 1.45; z-index: 10000; transition: opacity 0.18s ease; }
...
<div class="hero">
  <div class="hero-top">
    <div>
      ...
      <h2>Institutional Strategy Terminal</h2>
      <div class="hero-sub">
        Institutional analytics for the ADAPT CORE + ALPHA system.
...
with st.sidebar:
    ...
    st.markdown("""
    <div class="note-box">
    Top metrics and benchmark comparison are driven from the ADAPT return stream.
...
if run:
    ...
    <strong>Backtest Information</strong> &nbsp;|&nbsp;
    {strategy_choice} &nbsp;|&nbsp;
    {start_date} → {end_date} &nbsp;|&nbsp;
    {len(df):,} trading days &nbsp;|&nbsp;
    Rebalance Events: <strong>{rebalance_events:,}</strong>
    </div>
    """, unsafe_allow_html=True)
    ...
    with top_row_right:
        ...
        exec_tooltip = (
            "Signals are generated using final closing data.<br><br>"
            "Trades are assumed to be executed via the closing auction using "
            "<strong>Market-on-Close orders</strong>, not at the next day’s open."
        )
        st.markdown('<div class="mini-card">', unsafe_allow_html=True)
    ...
    with row2_left:
        rolling_tooltip = (
            "Rolling compound annual growth rate over moving time windows.<br><br>"
            "<strong>3Y CAGR</strong>: annualized return over rolling 3-year periods.<br>"
            "<strong>2Y CAGR</strong>: annualized return over rolling 2-year periods.<br><br>"
            "This shows the stability and variability of performance through time."
        )
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
    ...
    row3_left, row3_right = st.columns([0.92, 1.08])
    with row3_left:
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        st.markdown("#### Current Allocation")
        st.dataframe(current_alloc, use_container_width=True, hide_index=True)
    ...
    with st.expander("Validation / Audit Notes", expanded=False):
        ...
        with v2:
            ...
            st.write(f"Rebalance events: **{rebalance_events:,}**")
            st.write(f"2Y CAGR < 8%: **{prob_lt8:.2%}**")
            st.write(f"2Y CAGR < 5%: **{prob_lt5:.2%}**")

    st.caption('Note: CAGR, MaxDD, Sharpe, rolling CAGR, and underperformance probabilities are calculated from the raw return stream using df["ret"]. Final value and equity charts include scheduled contributions.')
else:
