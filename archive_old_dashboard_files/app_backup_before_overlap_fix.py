        ...
        for col in bench_ret.columns:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    text=bench_hover,
                    hovertemplate="%{text}<extra></extra>",
                    line=dict(width=1.8, color=benchmark_colors.get(col, "#94a3b8")),
                    opacity=0.68,
                ),
                row=1,
                col=1,
    ...
    if not rolling_2y.empty:
        fig.add_trace(
            go.Scatter(
                ...
                line=dict(color="#60a5fa", width=2.1, dash="dot"),
...
def build_allocation_fig(strategy_name: str, df: pd.DataFrame) -> go.Figure:
    ...
    keep_cols = alloc_df.mean().sort_values(ascending=False).head(8).index.tolist()
    alloc_df = alloc_df[keep_cols]

    colors = ["#fbbf24", "#60a5fa", "#34d399", "#c084fc", "#f87171", "#38bdf8", "#22c55e", "#f59e0b"]
    ...
    for i, col in enumerate(alloc_df.columns):
        fig.add_trace(
            go.Scatter(
                ...
                name=col,
                hovertemplate="Date: %{x|%Y-%m-%d}<br>%{fullData.name}: %{y:.1%}<extra></extra>",
            )
        )

    fig.update_layout(
        title=f"{strategy_name} Allocation Mix",
        height=360,
        template="plotly_dark",
        paper_bgcolor="#0f172a",
        plot_bgcolor="#0f172a",
...
def build_regime_fig(strategy_name: str, df: pd.DataFrame) -> go.Figure:
    ...
    regime_labels = {
        1: "R1 Risk-On",
        2: "R2 Bear",
        3: "R3 Growth Trending",
        4: "R4 Defensive",
    }
    regime_colors = {
        "R1 Risk-On": "#34d399",
        "R2 Bear": "#f87171",
        "R3 Growth Trending": "#60a5fa",
        "R4 Defensive": "#fbbf24",
    }
...
if run:
    ...
    allocation_fig = build_allocation_fig(strategy_choice, df)
    ...
    row3_left, row3_right = st.columns([1.1, 0.9])
    with row3_left:
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        st.plotly_chart(allocation_fig, use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)
