...
def contribution_flags(index: pd.Index, frequency: str) -> pd.Series:
    ...
    if frequency == "Monthly":
        ...
        flags.loc[~month_key.duplicated()] = True
    elif frequency == "Yearly":
        year_key = pd.Series(idx.to_period("Y"), index=idx)
        flags.loc[~year_key.duplicated()] = True
...
def allocation_change_log(strategy_name: str, df: pd.DataFrame, newest_first: bool = True) -> pd.DataFrame:
    ...
    changes = changes.sort_index(ascending=not newest_first if False else newest_first is False)
    if newest_first:
        changes = changes.sort_index(ascending=False)
    else:
        changes = changes.sort_index(ascending=True)
...
def estimate_trade_count(strategy_name: str, df: pd.DataFrame) -> int:
    alloc_df = build_allocation_frame(strategy_name, df)
    if alloc_df.empty or len(alloc_df) < 2:
        return 0

    rounded = alloc_df.round(6)
    prev = rounded.shift(1)
    changed = rounded.ne(prev) & prev.notna()
...
def build_equity_drawdown_fig(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            line=dict(width=3.2, color="#fbbf24"),
        ),
        row=1,
        col=1,
    )
        ...
        for col in bench_ret.columns:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    text=bench_hover,
                    hovertemplate="%{text}<extra></extra>",
                    line=dict(width=1.55, color=benchmark_colors.get(col, "#94a3b8")),
                    opacity=0.68,
                ),
                row=1,
                col=1,
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="lines",
            fill="tozeroy",
            line=dict(color="#f87171", width=1.9),
            name="Drawdown",
    ...
    fig.update_layout(
        ...
        paper_bgcolor="#0e1627",
        plot_bgcolor="#0e1627",
        margin=dict(l=24, r=24, t=78, b=20),
        legend=dict(
            orientation="h",
            yanchor="bottom",
            y=1.02,
    ...
    if not rolling_3y.empty:
        fig.add_trace(
            go.Scatter(
                ...
                line=dict(color="#34d399", width=2.25),
    ...
    if not rolling_2y.empty:
        fig.add_trace(
            go.Scatter(
                ...
                line=dict(color="#60a5fa", width=1.95, dash="dot"),
...
def build_regime_fig(strategy_name: str, df: pd.DataFrame) -> go.Figure:
    ...
    regime_labels = {1: "R1 Risk-On", 2: "R2 Bear", 3: "R3 Growth", 4: "R4 Defensive"}
    regime_colors = {
        "R1 Risk-On": "#34d399",
        "R2 Bear": "#f87171",
        "R3 Growth": "#60a5fa",
...
def build_yearly_heatmap(yr: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        ...
        paper_bgcolor="#0e1627",
        plot_bgcolor="#0e1627",
        margin=dict(l=24, r=24, t=24, b=20),
        xaxis_title="Year",
        yaxis_title="Series",
    )
    return fig
...
st.markdown(
    ...
    section[data-testid="stSidebar"] {
        background: linear-gradient(180deg, #09111f 0%, #0d1527 100%);
    ...
    .hero {
        background: linear-gradient(135deg, rgba(16,28,50,0.98) 0%, rgba(10,19,36,0.98) 100%);
    ...
    .floating-bar {
        ...
        top: 0.5rem;
        z-index: 999;
        background: linear-gradient(180deg, rgba(14,23,39,0.96) 0%, rgba(12,20,35,0.96) 100%);
    ...
    .section-card {
        background: linear-gradient(180deg, rgba(15,24,41,0.98) 0%, rgba(11,18,31,0.98) 100%);
        border: 1px solid var(--border);
        border-radius: 20px;
    ...
    .metric-shell {
        background: linear-gradient(180deg, rgba(17,27,46,0.96) 0%, rgba(13,21,37,0.98) 100%);
...
if run:
    ...
    trade_count = estimate_trade_count(strategy_choice, df)
    ...
    st.markdown(
        ...
        {start_date} → {end_date} &nbsp;|&nbsp;
        {len(df):,} trading days &nbsp;|&nbsp;
        Estimated allocation trades: <strong>{trade_count:,}</strong>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with top_row_left:
        ...
        <div class="metric-grid">
          ...
          <div class="metric-shell">
            <div class="metric-label">Estimated Trades</div>
            <div class="metric-value">{trade_count:,}</div>
            <div class="metric-sub">Allocation changes in selected period<br>See trade log below</div>
          </div>
        </div>
        """
    ...
    with top_row_right:
        ...
        exec_tooltip = (
            "Signals are generated using final closing data. "
            "Trades are assumed to be executed via the closing auction using "
            "Market-on-Close orders, not at the next day’s open."
        )
        st.markdown('<div class="mini-card">', unsafe_allow_html=True)
    ...
    with row2_right:
        if strategy_choice in {"CORE", "COMBINED"}:
            regime_tooltip = (
                "Distribution of time spent in each ADAPT allocation regime.<br><br>"
            ...
            st.markdown('<div class="section-card">', unsafe_allow_html=True)
            st.markdown(section_label("Regime Distribution", regime_tooltip), unsafe_allow_html=True)
    ...
    with st.expander("Validation / Audit Notes", expanded=False):
        ...
        with v2:
            ...
            st.write(f"Estimated trades: **{trade_count:,}**")
