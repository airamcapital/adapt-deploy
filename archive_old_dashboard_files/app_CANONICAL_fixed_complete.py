...
def section_label(title: str, tooltip: str) -> str:
    return (
        '<div class="tooltip-label"><span>' + title + '</span>'
        + '<span class="tooltip-wrap"><span class="tooltip-icon">i</span>'
        + '<span class="tooltip-text">' + tooltip + '</span></span></div>'
    )
...
def build_equity_drawdown_fig(strategy_name: str, df: pd.DataFrame, bench_ret: pd.DataFrame, start_value: float, contribution_amount: float, contribution_frequency: str, hover_mode: str = "Performance"):
    ...
    strategy_hover = [f"Date: {dt:%Y-%m-%d}<br>{strategy_name} Equity: ${eq:,.0f}<br><br>{hold}" if hover_mode == "Holdings %" else f"Date: {dt:%Y-%m-%d}<br>{strategy_name} Equity: ${eq:,.0f}" for dt, eq, hold in zip(df.index, strategy_equity_s, holdings_text)]
    ...
    if not bench_ret.empty:
        for col in bench_ret.columns:
            ...
            fig.add_trace(go.Scatter(x=bench_ret.index, y=b_eq, mode="lines", name=col, text=bench_hover, hovertemplate="%{text}<extra></extra>", line=dict(width=1.55), opacity=0.68), row=1, col=1)
...
def build_rolling_fig(returns: pd.Series):
    ...
    fig.update_layout(height=360, template="plotly_dark", paper_bgcolor="#0e1627", plot_bgcolor="#0e1627", margin=dict(l=24, r=24, t=40, b=20), yaxis_tickformat=".0%", xaxis_title="Date", yaxis_title="CAGR", legend=dict(orientation="h", yanchor="bottom", y=1.02, x=0))
...
if run:
    ...
    if df.empty:
        st.error("No strategy data for that range.")
        st.stop()
    ...
    with top_row_left:
        ...
        <div class="metric-grid">
          ...
          <div class="metric-shell"><div class="metric-label">Median Recovery</div><div class="metric-value">{0 if median_rec is None else median_rec:.0f} d</div><div class="metric-sub">Average <strong>{0 if avg_rec is None else avg_rec:.1f} d</strong></div></div>
