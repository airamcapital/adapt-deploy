...
if run:
    ...
    c7.markdown(metric_card_html("2Y Underperformance", f"{prob_neg:.2%}", ["Prob(2Y CAGR < 0%)", f"Median 2Y CAGR {roll2_med:.2%}"], tooltip=under_tooltip, align="left"), unsafe_allow_html=True)
    c8.markdown(metric_card_html("Sub-Target Risk", f"{prob_sub_target:.2%}", [f"Prob(<{sub_target_threshold:.1f}%) over rolling 2Y periods", f"Rebalance events {rebalance_events:,}"], tooltip=sub_target_tooltip, align="right"), unsafe_allow_html=True)
