...
def render_fill_tracking_panel(
    ...
    with c1:
        if st.button("Save Fill Tracking Log", width="stretch"):
            ...
            st.success(f"Fill log saved: {csv_path.name}")
...
def build_live_execution_snapshot(
    ...
    if not daily_ticket_df.empty and "Estimated Notional" in daily_ticket_df.columns:
        estimated_notional = (
            daily_ticket_df["Estimated Notional"]
            .astype(str)
    ...
    checklist_rows = [
        ...
        {"Check": "Execution cost estimate", "Status": "Pending", "Detail": "Reserved display slot; no new cost model added"},
    ]
...
def render_live_execution_panel(snapshot: dict, daily_ticket_df: pd.DataFrame):
    ...
    p3.markdown(
        metric_card_html(
            "Rebalance Today?",
            "Yes" if snapshot.get("rebalance_required", False) else "No",
            [f"Trade date {snapshot.get('latest_trade_date', 'N/A')}", f"Tickets {snapshot.get('trade_count', 0):,}"],
        ),
        unsafe_allow_html=True,
    )
    p4.markdown(
        metric_card_html(
            ...
            snapshot.get("estimated_cost_text", "Pending"),
            ["Reserved for next safe wiring step", "No new cost model added"],
        ),
        unsafe_allow_html=True,
    )
    p5.markdown(
    ...
    if not checklist_df.empty:
        st.dataframe(dark_dataframe(checklist_df), width="stretch")
    else:
...
def build_snapshot_payload(
    ...
    payload = {
        ...
        "execution_cost_estimate": snapshot.get("estimated_cost_text", "Pending"),
...
def render_snapshot_logging_panel(payload: dict):
    ...
    with c1:
        if st.button("Save Daily Snapshot", width="stretch"):
            ...
            st.success(f"Snapshot saved: {json_path.name}")
            st.caption(f"JSON: {json_path}")
            st.caption(f"Tickets CSV: {tickets_path}")
    with c2:
        st.download_button(
